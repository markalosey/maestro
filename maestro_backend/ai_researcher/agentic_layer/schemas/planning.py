...
class ReportSection(BaseModel):
    ...
    section_id: str = Field(
        ...,
        description="Unique identifier for the section (e.g., 'introduction', 'lit_review', 'methodology_results', 'conclusion').",
    )
    title: str = Field(..., description="Human-readable title for the report section.")
    description: str = Field(
        ...,
        description="Detailed description of what this section should cover, including specific subtopics and questions to address.",
    )
    associated_note_ids: Optional[List[str]] = Field(
        default=None,
        description="List of note IDs from the exploratory phase relevant to this section.",
    )  # Added field
    subsections: List["ReportSection"] = Field(
        default_factory=list,
        description="List of subsections nested under this section.",
    )
    # --- NEW FIELDS ---
...
class SimplifiedSection(BaseModel):
    ...
    section_id: Optional[str] = Field(
        None,
        description="Unique identifier for the section (e.g., 'introduction', 'section_1').",
    )
    title: str = Field(..., description="Human-readable title for the report section.")
    description: str = Field(
        ...,
    ...
    associated_note_ids: Optional[List[str]] = Field(
        None,
        description="List of note IDs associated with this section (for Phase 3 revision).",
    )
    subsections: List["SimplifiedSection"] = Field(
        default_factory=list,
        description="List of subsections nested under this section (max depth of 2).",
    )
...
class SectionWithNotes(BaseModel):
    ...
    research_strategy: ResearchStrategy = Field(
        ..., description="Research strategy for this section."
    )
    associated_note_ids: List[str] = Field(
        default_factory=list,
        description="List of note IDs from research that are relevant to this section.",
    )
    subsections: List["SectionWithNotes"] = Field(
...
class Phase1PlanResponse(BaseModel):
    ...
    mission_goal: str = Field(
        ..., description="The overall research goal restated clearly."
    )
    generated_thought: str = Field(
        ...,
        description="Your analytical thought about the research task and your planning approach.",
    )
    report_outline: List[SimplifiedSection] = Field(
        ..., description="The planned structure of the report."
    )
    parsing_error: Optional[str] = Field(
        None,
        description="Field to indicate if there was an error parsing the LLM response into the schema.",
    )
...
class Phase2PlanResponse(BaseModel):
    ...
    generated_thought: str = Field(
        ..., description="Your thought about how the notes map to the outline."
    )
    report_outline: List[SectionWithNotes] = Field(
        ..., description="The outline with notes assigned to sections."
    )
    parsing_error: Optional[str] = Field(
        None,
        description="Field to indicate if there was an error parsing the LLM response into the schema.",
...
class Phase3PlanResponse(BaseModel):
    ...
    mission_goal: str = Field(..., description="The overall research goal.")
    generated_thought: str = Field(
        ..., description="Your thought about the revisions made."
    )
    report_outline: List[SimplifiedSection] = Field(
        ..., description="The revised outline structure."
    )
    parsing_error: Optional[str] = Field(
        None,
        description="Field to indicate if there was an error parsing the LLM response into the schema.",
...
class Phase3aStructuralResponse(BaseModel):
    ...
    mission_goal: str = Field(..., description="The overall research goal.")
    generated_thought: str = Field(
        ..., description="Summary of structural changes applied."
    )
    report_outline: List[SimplifiedSection] = Field(
        ..., description="The structurally modified outline."
    )
    modifications_applied: List[str] = Field(
        default_factory=list,
        description="List of modification types that were successfully applied.",
    )
...
class Phase3bSubsectionResponse(BaseModel):
    ...
    mission_goal: str = Field(..., description="The overall research goal.")
    generated_thought: str = Field(
        ..., description="Summary of subsections added and notes assigned."
    )
    report_outline: List[SectionWithNotes] = Field(
        ..., description="The outline with new subsections and note assignments."
    )
...
class Phase3cNoteRedistributionResponse(BaseModel):
    ...
    report_outline: List[SectionWithNotes] = Field(
        ..., description="The outline with finalized note assignments."
    )
...
class StepParameters(BaseModel):
    ...
    query: Optional[str] = Field(
        None, description="Search query for document_search or web_search"
    )
    ...
    max_results: Optional[int] = Field(
        None, description="Maximum results for web search"
    )
    filepath: Optional[str] = Field(
        None, description="File path for read_full_document"
    )
...
class PlanStep(BaseModel):
    model_config: ClassVar[ConfigDict] = ConfigDict(
        extra="forbid"
    )  # Enforce additionalProperties: false in schema
    """Defines a single step in the research plan."""
    step_id: str = Field(
        ..., description="Unique identifier for this step (e.g., 'step_1', 'step_2a')."
    )
    action_type: ActionType = Field(
        ..., description="The type of action to perform for this step."
    )
    description: str = Field(
        ..., description="Detailed description of what needs to be done in this step."
    )
    parameters: StepParameters = Field(
        default_factory=StepParameters
    )  # Remove description to avoid $ref conflict
    depends_on: List[str] = Field(
        default_factory=list,
        description="List of step_ids that must be completed before this step can start.",
    )
    produces: Optional[str] = Field(
        None,
        description="Optional description of the expected output or artifact from this step (e.g., 'list of relevant paper snippets', 'calculated value', 'synthesized findings on topic X').",
    )
...
class SimplifiedPlan(BaseModel):
    ...
    report_outline: List[ReportSection] = Field(
        ..., description="The planned structure of the final report."
    )
...
class SimplifiedPlanResponse(BaseModel):
    ...
    generated_thought: Optional[str] = Field(
        None,
        description="A concise thought or reminder generated by the agent to be added to the thought_pad.",
    )
